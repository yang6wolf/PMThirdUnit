                                  ...
                                  LDNetConnectDelegate>
{
    ...
    BOOL _isRecordOutIPInfoOpened;
    
    // Connect设置相关
    NSUInteger _connectMaxCount; // 最大Connect次数，初始默认4次
    double _connectTimeOutTime; // 最大超时响应时间，初始默认3秒
}
        ...
        MobileNetCode:(NSString *)theMobileNetCode
{
    ...
    if (self) {
        ...
        _isRunning = NO;
        _isRecordOutIPInfoOpened = NO;
        
        _connectMaxCount = 4;
        _connectTimeOutTime = 3.0;
    }

    return self;
}

- (void)setConnectMaxCount:(NSUInteger)connectMaxCount
        connectTimeOutTime:(double)connectTimeOutTime;
{
    _connectMaxCount = connectMaxCount;
    _connectTimeOutTime = connectTimeOutTime;
}
...
- (void)startNetDiagnosis
{
    ...
    if (_isRunning) {
        ...
        if ([_hostAddress count] > 0) {
            ...
            [_netConnect setMaxCount:_connectMaxCount timeOutTime:_connectTimeOutTime];
...
- (void)isOpenRecordOutIPInfo:(BOOL)opened
{
    _isRecordOutIPInfoOpened = opened;
}
@end
