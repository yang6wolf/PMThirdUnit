...
@interface LDNetConnect () {
    ...
    NSUInteger _maxCount;   // 最大连接次数, 初始化为4次
    double _timeOutTime;     // 超时响应时间, 初始化为3.0s
}
...
-(void)connectWithAddress:(NSData *)addr{
    ...
    //执行连接
    CFSocketConnectToAddress(_socket, (__bridge CFDataRef)addr, _timeOutTime);
